$(document).ready(function () {
   ...
   $(".series .show ul.cont li").mouseover(function () {
      ...
      // console.log($('.series .show .cont li').eq(index).find('img').attr('src'));
      $simg.find("img").attr("src", $('.series .show .cont li').eq(index).find('img').attr('src'));
      $bigImg.find("img").attr("src", $('.series .show .cont li').eq(index).find('img').attr('src'));
   });
   ...
   $simg.hover(function () {
      ...
      $simg.mousemove(function (e) {
         ...
         if (_left < 0) { _left = 0; }
         else if (_left > simgMovW) { _left = simgMovW; }
         if (_top < 0) { _top = 0; }
         else if (_top > simgMovH) { _top = simgMovH; }
